...
- ESP01S and `att_network` are build-linked only; WiFi/TCP startup remains gated behind later network scheduling work.

## Not Done

- FreeRTOS tasks do not yet run ESP01S upload logic.
- RC522 UID read and card block read/write are build-linked but not real-board validated.
- ESP01S WiFi, TCP, NTP, weather and heartbeat paths are build-linked but not real-board validated.
