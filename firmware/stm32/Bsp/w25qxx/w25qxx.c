...
// 	printf("ad:%X,nb:%X\r\n",WriteAddr,NumByteToWrite);//测试用
    ...
    while(1)
    {
        W25QXX_Read(W25QXX_BUF,secpos*SECTOR_SIZE,SECTOR_SIZE);//读出整个扇区的内容
        ...
        if(i<secremain)//需要擦除
        {
            ...
            W25QXX_Write_NoCheck(W25QXX_BUF,secpos*SECTOR_SIZE,SECTOR_SIZE);//写入整个扇区
        }
        ...
        else//写入未结束
        {
            ...
            if(NumByteToWrite>SECTOR_SIZE)secremain=SECTOR_SIZE;	//下一个扇区还是写不完
